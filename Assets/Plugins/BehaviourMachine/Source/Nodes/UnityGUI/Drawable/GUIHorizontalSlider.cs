...
namespace BehaviourMachine {
    ...
    /// <summary>
    /// A horizontal slider the user can drag to change a value between a min (Left Value) and a max (Right Value).
    /// </summary>
    [NodeInfo(  category = "UnityGUI/Drawable/GUI/",
                icon = "GUIText",
                description = "A horizontal slider the user can drag to change a value between a min (Left Value) and a max (Right Value)",
                url = "http://docs.unity3d.com/Documentation/ScriptReference/GUI.HorizontalSlider.html")]
    public class GUIHorizontalSlider : GUIRectNode, IGUINode {
    	...
    	public override Status Update () {
            ...
            value.Value = GUI.HorizontalSlider(this.GetRect(), value.Value, leftValue.Value, rightValue.Value);
