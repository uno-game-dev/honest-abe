...
namespace BehaviourMachine {
    ...
    [NodeInfo(  category = "UnityGUI/Drawable/GUI/",
                ...
                url = "http://docs.unity3d.com/Documentation/ScriptReference/GUI.TextField.html")]
    public class GUITextField : GUIRectNode {
    	...
    	public override Status Update () {
            ...
            if (maxLength.isNone) {
                if (guiStyle.isNone)
                    text.Value = GUI.TextField(this.GetRect(), text.Value);
                else
                    text.Value = GUI.TextField(this.GetRect(), text.Value, guiStyle.Value);
            }
            else {
                if (guiStyle.isNone)
                    text.Value = GUI.TextField(this.GetRect(), text.Value, maxLength.Value);
                else
                    text.Value = GUI.TextField(this.GetRect(), text.Value, maxLength.Value, guiStyle.Value);
            }
