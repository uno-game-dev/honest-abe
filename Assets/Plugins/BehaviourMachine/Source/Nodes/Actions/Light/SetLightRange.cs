//----------------------------------------------
//            Behaviour Machine
// Copyright © 2014 Anderson Campos Cardoso
//----------------------------------------------

using UnityEngine;
using System.Collections;

namespace BehaviourMachine {

    /// <summary>
    /// Sets the range of a light.
    /// </summary>
    [NodeInfo ( category = "Action/Light/",
                icon = "Light",
                description = "Sets the range of a light",
                url = "http://docs.unity3d.com/Documentation/ScriptReference/Light-range.html")]
    public class SetLightRange : ActionNode {
        /// <summary>
        /// The game object that has a light in it.
        /// </summary>
        [VariableInfo(requiredField = false, nullLabel = "Use Self", tooltip = "The game object that has a light in it")]
        public GameObjectVar gameObject;

        /// <summary>
        /// The new range of the light.
        /// </summary>
        [VariableInfo(tooltip = "The new range of the light")]
        public FloatVar newRange;

        public override void Reset () {
            gameObject = new ConcreteGameObjectVar(this.self);
            newRange = new ConcreteFloatVar();
        }

        public override Status Update () {
            // Get the light
            Light light = gameObject.Value != null? gameObject.Value.GetComponent<Light>() : null;

            // Validate members
            if (light == null || newRange.isNone)
                return Status.Error;

            // Set light range
            light.range = newRange.Value;
            return Status.Success;
        }
    }
}